...
from authlib.oauth2.rfc6749.grants.base import (
    BaseGrant,
...
class AuthorizationCodeGrant(BaseGrant, AuthorizationEndpointMixin, TokenEndpointMixin):
    ...
    def create_authorization_response(self, redirect_uri, grant_user, req):
        ...
        if hasattr(self, "create_authorization_code"):  # pragma: no cover
            ...
            self.save_authorization_code(code, self.request, req)
    ...
    def validate_token_request(self, req):
        ...
        authorization_code = self.query_authorization_code(code, client, req)
    ...
    def create_token_response(self, req):
        ...
        user = self.authenticate_user(authorization_code, req)
        if not user:
        ...
        self.delete_authorization_code(authorization_code, req)
        return {"token": token, "header": self.TOKEN_RESPONSE_HEADER, "status": 200}
    ...
    def query_authorization_code(self, code, client, request):  # pragma: no cover
    ...
    def authenticate_user(self, authorization_code, request):
