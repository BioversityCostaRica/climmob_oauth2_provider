...
class AuthorizationServer(object):
    ...
    def get_token_grant(self, request):
        ...
        for (grant_cls, extensions) in self._token_grants:
            ...
            if (
                grant_cls.check_token_endpoint(request)
                and request.method in grant_cls.TOKEN_ENDPOINT_HTTP_METHODS
            ):
                return _create_grant(grant_cls, extensions, request, self)
    ...
    def create_authorization_response(self, req=None, grant_user=None):
        ...
        :returns: Response
        """
        request = self.create_oauth2_request(req)
        try:
        ...
        try:
            ...
            args = grant.create_authorization_response(redirect_uri, grant_user, req)
            return args
    ...
    def create_token_response(self, req):
        ...
        try:
            grant.validate_token_request(req=req)
            args = grant.create_token_response(req=req)
            return args
        except OAuth2Error as error:
            return self.handle_error_response(request, error)
